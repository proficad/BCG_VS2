...
{
	if (!CBCGPMDIFrameWnd::LoadFrame(IDR_BCGVS2TYPE, dwDefaultStyle, pParentWnd, pContext))
	{
		return FALSE;
	}
...
{
	const int li_count = m_wndMenuBar.GetAllButtons().GetCount();
	for (int li_i = 0; li_i < li_count; li_i++)
	{
		CBCGPToolbarButton* l_button = m_wndMenuBar.GetButton(li_i);
		
		CString ls_old = l_button->m_strText;
		l_button->m_strText = ls_old + L" translated";
		m_wndMenuBar.UpdateButton(li_i);
	}

	m_wndMenuBar.AdjustSizeImmediate(TRUE);
}
